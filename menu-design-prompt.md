...
你是一個專業的網頁平面設計師。附件 `menu` 是一家泰式餐廳的手寫菜單，請將它設計成 **手機瀏覽器專用** 的線上菜單頁面。

這是一個給顧客在店內掃 QR code 使用的 **真實餐廳菜單**，不是外送平台、不是購物車流程、不是點餐結帳介面，也不是品牌宣傳 landing page。請以 **真實可上線、好閱讀、好操作** 為優先。

請設計成 **兩個主要畫面**：
1. 主菜單頁（Main Menu）
2. 品項詳情 Overlay / Modal（Item Detail Modal）

不需要考慮桌機或平板版面，只需以 **375px 寬的手機畫面** 為基準。

---

## 一、整體產品定位

這個頁面要像一家餐廳真的會上線給客人使用的手機菜單，而不是設計作品集或概念稿。

設計目標：
- 真實、自然、可上線
- 容易快速掃讀品項與價格
- 有一點泰式餐廳的溫度，但不要過度裝飾
- 以使用性優先，不要做成點餐 app 或購物流程

如果需要參考模板，僅可參考 **最中性的 mobile content / list 類型 template 作為版面骨架**，不要套用 food delivery、ecommerce、checkout、dashboard 類型模板。
...
- 形象圖片（若有提供則顯示，否則先以色塊或簡潔視覺區塊佔位）
- 用餐資訊（需隨語言切換）：
  - 用餐時間限 90 分鐘
  - 每人最低消費 NT$300

店家資訊區塊應簡潔、有辨識度，但不要做成過大的 hero banner，避免像宣傳首頁。重點是讓使用者很快理解這是一份菜單並開始瀏覽。

---

## 三、設計規範

- **配色**：以菜單上的金黃色（皇冠推薦圖示色系）為主色，搭配白底、深灰文字。如工作區內已有色彩元素則以工作區為準。
- **色彩使用方式**：金黃色請節制使用，作為重點強調色，不要整頁大面積鋪滿。
- **字體**：中文、日文、韓文使用無襯線字體；英文同樣無襯線。
- **資訊層級**：品項名稱字級應明顯大於價格、規格、標籤。
- **僅限手機**：以 375px 寬為基準設計，不需響應式設計桌面或平板。
...
可考慮在主頁加入 **sticky 分類快捷列 / tab bar**，讓使用者能快速切換不同分類。

---

## 六、品項卡片

每個品項以卡片或輕量列表形式呈現，重點是讓使用者可以快速掃讀菜名、價格、推薦與辣度資訊。

每個品項需包含：

1. **照片**
   - 若有圖片則顯示
   - 若 `image` 為 null，以放大的 `emoji` 顯示代替
...
3. **價格 / 規格**
   - 單一價格：直接顯示 `NT$95`
   - 多規格（大/小、杯/壺、不同主食）：在同一卡片內清楚呈現規格名稱與價格
...
標記應以「資訊輔助」為主，不要讓整個畫面變成 badge collection。
...
### 9-2. 完整渲染格式（由建置工具或 AI 從 menu-raw.js 自動產生）
